#MLR with PC data and MEI, PDO & PNA indices AND Global Temperature Data (1920-2012)
...
for (i in 1:4) {
  ...
  for (j in 1:2) {
    ...
    #***************Additive models:
    #PC1:
    fit1a<-lm(pc_ts$PC1~MEI+PDO+PNA+temp)
    ...
    #PC2:
    fit2a<-lm(pc_ts$PC2~MEI+PDO+PNA+temp)
    ...
    #PC3:
    fit3a<-lm(pc_ts$PC3~MEI+PDO+PNA+temp)
    ...
    #PC4:
    fit4a<-lm(pc_ts$PC4~MEI+PDO+PNA+temp)
    ...
    #output anova tables:
    setwd("C:/AbbyF/Oliver/MLR/NEW_TS/temp/")
