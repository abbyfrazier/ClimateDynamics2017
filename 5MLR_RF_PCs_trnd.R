#MLR with PC data and MEI, PDO & PNA indices AND a linear trend term
#Using PCA of rainfall maps

#1920-2012
#....it will delete any that don't match, so really it's 1950-2012

#TRy with slope -20% and -5% (just change "trnd20" to "trnd05")
#**update - no difference between using slope 20 and slope 5...MLR scales them the same.
...
for (i in 1:4) {
  ...
  for (j in 1:2) {
    ...
    #***************Additive models:
    #PC1:
    fit1a<-lm(pc_ts$PC1~MEI+PDO+PNA+trnd)
    ...
    #PC2:
    fit2a<-lm(pc_ts$PC2~MEI+PDO+PNA+trnd)
    ...
    #PC3:
    fit3a<-lm(pc_ts$PC3~MEI+PDO+PNA+trnd)
    ...
    #PC4:
    fit4a<-lm(pc_ts$PC4~MEI+PDO+PNA+trnd)
    ...
    #output anova tables:
    setwd("C:/AbbyF/Oliver/MLR/NEW_TS/LinearTrend/trnd05/")
